...
加密我使用异或后取反，解密则是相反（异或的逆运算是其自身）。注意加密函数 `EncryptChar` 带的是 `constexpr`，而解密函数 `DecryptChar` 则没有，因为解密操作需要在运行期进行，带 `inline` 说明符是因为函数定义在头文件里，用于保证被不同源文件（.cpp）引入后只有一份定义。（是的，`inline` 的含义在Modern C++中已经变了。）
